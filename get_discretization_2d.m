function [discretization, integrator] = get_discretization_2d(problem, p, ne, Tmax, method, product, flag_mex)
    ...
    discretization.quadrature.sumfact = {zeros(m), zeros(m), zeros(m)};
    
    % precompute geometric particulars, including first and second derivatives
    discretization.geometry = geometric_particulars_2d(problem.geometry.mapping, discretization.quadrature);
    ...
    if strcmp(discretization.product, 'standard')
        if strcmp(discretization.method, 'standard')
            discretization.mass = assemble_mass_matrix_2d(discretization, flag_mex);
            discretization.chol = chol(discretization.mass);
        elseif strcmp(discretization.method, 'lumped')
            discretization.approxinverse = reshape(1 ./ sum(assemble_mass_matrix_2d(discretization, flag_mex), 2), discretization.basis{1}.dim, discretization.basis{2}.dim);
        else
    ...
    omega_max = estimate_maximum_eigenvalue_2d(discretization, problem, flag_mex);
